  ...
  return (
    <div className="site-shell"><Header /><div className="main-pane"><main className="page-wrap">
      <section className="grid gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-teal-200 bg-white p-4">
          <div>
        ...
        <form onSubmit={handleSubmit(saveResume, (formErrors) => toast.error(extractFirstIssue(formErrors)))} className="grid gap-4">
          <div className="grid gap-4 xl:grid-cols-[1.1fr_0.9fr]"><section className="rounded-2xl border border-teal-200 bg-white p-4"><h2 className="text-lg font-bold text-slate-900">Contato</h2><div className="mt-5 grid gap-4"><div className="grid gap-2"><label className="text-sm font-semibold text-slate-800">E-mail</label><Input {...register("email")} type="email" placeholder="nome@exemplo.com" />{errors.email && <p className="error-text">{errors.email.message?.toString()}</p>}</div><div className="grid gap-2"><label className="text-sm font-semibold text-slate-800">Telefone</label><Controller name="telefone" control={control} render={({ field }) => (<IMaskInput {...field} mask="(00) 00000-0000" placeholder="(99) 99999-9999" className="input-shell" />)} />{errors.telefone && <p className="error-text">{errors.telefone.message?.toString()}</p>}</div><div className="grid gap-2"><label className="text-sm font-semibold text-slate-800">CPF</label><Controller name="cpf" control={control} render={({ field }) => (<IMaskInput {...field} mask="000.000.000-00" placeholder="000.000.000-00" className="input-shell" />)} />{errors.cpf && <p className="error-text">{errors.cpf.message?.toString()}</p>}</div></div></section><div className="grid gap-4"><section className="rounded-2xl border border-teal-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-900">Informacoes principais</h2><p className="mt-1 text-sm text-slate-600">Dados de apresentacao do currículo.</p></div><span className="inline-flex rounded-full px-3 py-1 text-xs font-bold bg-teal-100 text-teal-900">Obrigatorio</span></div><div className="mt-5 grid gap-4 lg:grid-cols-2"><div className="grid gap-2"><label className="text-sm font-semibold text-slate-800">Nome</label><Input {...register("nome")} type="text" placeholder="Nome completo" />{errors.nome && <p className="error-text">{errors.nome.message?.toString()}</p>}</div><div className="grid gap-2"><label className="text-sm font-semibold text-slate-800">Cargo desejado</label><Input {...register("cargo")} type="text" placeholder="Cargo desejado" />{errors.cargo && <p className="error-text">{errors.cargo.message?.toString()}</p>}</div></div><div className="mt-4 grid gap-2"><label className="text-sm font-semibold text-slate-800">Resumo profissional</label><Textarea {...register("resumo")} placeholder="Descreva experiencia e competencias" />{errors.resumo && <p className="error-text">{errors.resumo.message?.toString()}</p>}</div></section><section className="rounded-2xl bg-teal-50 p-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-lg font-bold text-slate-900">Imagem do candidato</h2><p className="mt-1 text-sm text-slate-600">Upload visual para compor a ficha.</p></div><span className="inline-flex rounded-full px-3 py-1 text-xs font-bold border border-teal-200 text-slate-700">Opcional</span></div><div className="mt-5 grid gap-4 sm:grid-cols-[auto_1fr] sm:items-center"><div className="relative h-24 w-24 overflow-hidden rounded-2xl border border-teal-200 bg-white"><Image src={previewImage} alt="Avatar" fill className="object-cover" /></div><div className="grid gap-2"><label className="text-sm font-semibold text-slate-800">Arquivo</label><Controller name="avatar" control={control} render={({ field }) => (<input type="file" accept="image/*" onChange={(event) => { field.onChange(event.target.files); syncAvatarPreview(event); }} className="input-shell file:mr-4 file:rounded-full file:border-0 file:bg-slate-900 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-white" />)} /></div></div></section></div></div><div className="grid gap-4 xl:grid-cols-[0.8fr_1.2fr]"><section className="rounded-2xl bg-teal-50 p-4"><h2 className="text-lg font-bold text-slate-900">Habilidades</h2><div className="mt-5 grid gap-2"><label className="text-sm font-semibold text-slate-800">Lista de habilidades</label><Input {...register("habilidades")} type="text" placeholder="React, Next.js, TypeScript" /><p className="text-xs text-slate-500">Separe por virgula.</p>{errors.habilidades && <p className="error-text">{errors.habilidades.message?.toString()}</p>}</div></section><div className="grid gap-4"><section className="rounded-2xl border border-teal-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-bold text-slate-900">Formacao academica</h2><Button type="button" variant="secondary" onClick={() => studyCollection.append({ instituicao: "", curso: "", periodo: "" })}>Adicionar</Button></div><div className="mt-5 grid gap-4">{studyCollection.fields.map((field, index) => (<div key={field.id} className="rounded-xl border border-teal-200 p-4"><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold text-slate-700">Formacao {index + 1}</span><Button type="button" variant="danger" onClick={() => studyCollection.remove(index)}>Remover</Button></div><div className="mt-4 grid gap-4"><Input {...register(`formacoes.${index}.instituicao` as const)} type="text" placeholder="Instituicao" /><div className="grid gap-4 lg:grid-cols-2"><Input {...register(`formacoes.${index}.curso` as const)} type="text" placeholder="Curso" /><Input {...register(`formacoes.${index}.periodo` as const)} type="text" placeholder="Periodo" /></div></div></div>))}{errors.formacoes && <p className="error-text">{extractFirstIssue(errors.formacoes)}</p>}</div></section><section className="rounded-2xl border border-teal-200 bg-white p-4"><div className="flex items-center justify-between gap-3"><h2 className="text-lg font-bold text-slate-900">Experiencias profissionais</h2><Button type="button" variant="secondary" onClick={() => workCollection.append({ empresa: "", cargo: "", periodo: "", descricao: "" })}>Adicionar</Button></div><div className="mt-5 grid gap-4">{workCollection.fields.map((field, index) => (<div key={field.id} className="rounded-xl border border-teal-200 p-4"><div className="flex items-center justify-between gap-3"><span className="text-sm font-semibold text-slate-700">Experiencia {index + 1}</span><Button type="button" variant="danger" onClick={() => workCollection.remove(index)}>Remover</Button></div><div className="mt-4 grid gap-4 lg:grid-cols-2"><Input {...register(`experiencias.${index}.empresa` as const)} type="text" placeholder="Empresa" /><Input {...register(`experiencias.${index}.cargo` as const)} type="text" placeholder="Cargo" /></div><div className="mt-4 grid gap-4 lg:grid-cols-2"><Input {...register(`experiencias.${index}.periodo` as const)} type="text" placeholder="2021 - 2024" /><Textarea {...register(`experiencias.${index}.descricao` as const)} placeholder="Descricao" /></div></div>))}{errors.experiencias && <p className="error-text">{extractFirstIssue(errors.experiencias)}</p>}</div></section></div></div>
